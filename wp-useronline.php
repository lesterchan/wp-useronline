...
function useronline_menu() {
	if (function_exists('add_submenu_page')) {
		add_submenu_page('index.php',  __('WP-UserOnline', 'wp-useronline'),  __('WP-UserOnline', 'wp-useronline'), 1, 'wp-useronline/useronline.php', 'display_useronline');
	}
...
function useronline_header() {
	...
	wp_register_script('wp-useronline', '/wp-content/plugins/wp-useronline/useronline-js.php', false, '2.30');
	wp_print_scripts(array('sack', 'wp-useronline'));
	echo '<!-- End Of Script Generated By WP-UserOnline 2.30 -->'."\n";
}
...
function useronline_page() {
	...
	if($bots) {
		foreach($bots as $bot) {
			$useronline_output .= '<p><strong>#'.$no.' - '.$bot['display_name'].'</strong> '.check_ip($bot['ip']).' '.__('on', 'wp-useronline').' '.mysql2date(sprintf(__('%s @ %s', 'wp-useronline'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $bot['timestamp'])).'<br />'.$bot['location'].' [<a href="'.$bot['url'].'">'.__('url', 'wp-useronline').'</a>]</p>'."\n";
			$no++;
		}
	}
