...
* FIXED: A single address could fill the table and permanently inflate "most users ever online" — a figure with no reset anywhere — by varying its user agent on each request, which defeated the de-duplication and left one row per second standing for the whole timeout. At most twenty rows per address are kept now; `wp_useronline_max_per_address` raises the ceiling, or zero removes it
